...
const ALL_ROLE_SKILL_IDS=new Set(ROLE_SKILLS.flatMap(r=>r.skills));
const EXTRA={
...
function renderTechnical(state){const box=$('#compTechnicalList');if(!box)return;const items=Object.values(state.technical||{}).sort((a,b)=>String(a.category||'').localeCompare(String(b.category||''))||String(a.label||'').localeCompare(String(b.label||'')));box.innerHTML=items.length?items.map(x=>`<label class="compSkillCard ${x.confirmed?'confirmed':''}"><input type="checkbox" data-sync-tech="${esc(x.id)}" ${x.confirmed?'checked':''}><span><b>${esc(x.label)}</b><small>${esc(x.category||'Technical skill')} • ${esc(sourceText(x))}</small>${x.evidence?.[0]?`<em>${esc(x.evidence[0])}</em>`:''}</span></label>`).join(''):'<div class="empty">No technical or workplace skills surfaced yet. Add résumé evidence, confirm occupation suggestions, or add one manually.</div>';box.querySelectorAll('[data-sync-tech]').forEach(el=>el.onchange=()=>{const x=state.technical[el.dataset.syncTech];if(!x)return;x.confirmed=el.checked;x.sources=x.sources||[];if(el.checked&&!x.sources.includes('résumé')&&!x.sources.includes('participant'))x.sources.push('participant');if(!el.checked&&x.sources.includes('participant')&&x.sources.includes('occupation suggestion'))x.sources=x.sources.filter(s=>s!=='participant');write(state);renderTechnical(state);renderMarket(state)})}
function hasIndependentEvidence(x){const s=x.sources||[];return s.includes('résumé')||s.includes('participant')||s.includes('participant-added skill')||((x.evidence||[]).length>0&& !s.includes('occupation suggestion'))}
function sync(){const role=$('#currentRole')?.value.trim()||'';const allowed=allowedFor(role);const state=read();state.technical=state.technical||{};state.marketNow=state.marketNow||[];
 for(const [id,x] of Object.entries(state.technical)){
   x.sources=x.sources||[];
   const isKnownRoleSkill=ALL_ROLE_SKILL_IDS.has(id);
   const wasOccupationSuggested=x.sources.includes('occupation suggestion');
   const legacyUnattributedRoleSkill=isKnownRoleSkill&&!x.sources.length;
   if(allowed.has(id)){
     if(isKnownRoleSkill&&!hasIndependentEvidence(x)&&!x.sources.includes('occupation suggestion'))x.sources.push('occupation suggestion');
     continue;
   }
   if(!wasOccupationSuggested&&!legacyUnattributedRoleSkill)continue;
   x.sources=x.sources.filter(s=>s!=='occupation suggestion');
   if(!hasIndependentEvidence(x)){
     delete state.technical[id];
     state.marketNow=state.marketNow.filter(k=>k!=='tech:'+id);
   }
 }
 for(const id of allowed){
   if(state.technical[id]){
     const x=state.technical[id];x.sources=x.sources||[];
     if(!hasIndependentEvidence(x)&&!x.sources.includes('occupation suggestion'))x.sources.push('occupation suggestion');
     continue;
   }
   if(EXTRA[id])state.technical[id]=JSON.parse(JSON.stringify(EXTRA[id]));
 }
 state.updatedAt=new Date().toISOString();write(state);renderTechnical(state);renderMarket(state)}
function install(){const input=$('#currentRole');if(!input)return;let t;const queue=()=>{clearTimeout(t);t=setTimeout(sync,320)};input.addEventListener('input',queue);input.addEventListener('change',queue);document.addEventListener('click',e=>{if(e.target.closest?.('[data-occ-index]'))setTimeout(sync,80)},true);setTimeout(sync,420)}
