(function(){
  ...
  function safeStorageSnapshot(){const out={};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(!k||k===J||k===ID||k===TOK||k.startsWith('sb-')||k.includes('access_token')||k.includes('refresh_token'))continue;let v=localStorage.getItem(k);if(v!=null&&v.length<=200000)out[k]=v}return out}
  function captureModule(moduleId,extra){const j=get();j.progress=j.progress||{};j.modules=j.modules||{};j.progress[moduleId]='complete';j.modules[moduleId]=Object.assign({},j.modules[moduleId]||{},extra||{},{completedAt:new Date().toISOString(),browserState:safeStorageSnapshot()});return put(j)}
