(function(){
  ...
  function restoreBrowserState(j){const modules=j?.modules||{};const ordered=Object.values(modules).filter(x=>x&&x.browserState).sort((a,b)=>String(a.completedAt||'').localeCompare(String(b.completedAt||'')));for(const mod of ordered){for(const [k,v] of Object.entries(mod.browserState||{})){if(isSafeStorageKey(k)&&typeof v==='string')localStorage.setItem(k,v)}}}
  async function loadMine(){const cl=c();if(!cl)return null;const {data:{session}}=await cl.auth.getSession();if(!session?.user)return null;const {data,error}=await cl.rpc('pinal_boost_load_my_journey');if(error){console.warn('Pinal BOOST load failed',error.message);return null}if(data){localStorage.setItem(J,JSON.stringify(data));restoreBrowserState(data);return data}return null}
